...
if __name__ == "__main__":
    ...
    # Test 1: Create a valid party
    print("1. Creating test party...")
    party = create_test_party(1, "Brave Companions")
    print(f"   {party}")
    print(f"   Initial morale: {party.calculate_morale()}")
    
    # Test 2: Test role-based member lookup
    print("\n2. Testing role-based lookup...")
    striker = party.get_member_by_role(CharacterRole.STRIKER)
    burglar = party.get_member_by_role(CharacterRole.BURGLAR)
    print(f"   Striker: {striker.name}")
    print(f"   Burglar: {burglar.name}")
    
    # Test 3: Test damage and morale
    print("\n3. Testing damage and morale changes...")
    print(f"   Before damage - Morale: {party.calculate_morale()}")
    striker.take_damage(10)  # Striker takes damage
    burglar.disable_spell("Detect Traps")  # Burglar loses a spell
    print(f"   After damage - Morale: {party.calculate_morale()}")
    print(f"   {party}")
    
    # Test 4: Test expedition progress
    print("\n4. Testing expedition progress...")
    party.add_gold(150)
    party.complete_room()
    party.complete_room()
    party.complete_floor()
    party.defeat_monsters(3)
    print(f"   {party}")
    
    # Test 5: Test party wipe
    print("\n5. Testing party wipe scenario...")
    for member in party.members:
        member.take_damage(30)  # Down everyone
    print(f"   Party wiped: {party.is_party_wiped()}")
    print(f"   Alive members: {len(party.alive_members())}")
    
    # Test 6: Test expedition reset
    print("\n6. Testing expedition reset...")
    party.reset_for_new_expedition()
    print(f"   After reset: {party}")
    print(f"   All alive: {len(party.alive_members()) == 4}")
    
    # Test 7: Test invalid party composition
    print("\n7. Testing invalid party composition...")
    try:
        # Try to create party with 2 strikers
        bad_members = [
            Character("Striker1", CharacterRole.STRIKER, 1),
            Character("Striker2", CharacterRole.STRIKER, 1),
            Character("Support1", CharacterRole.SUPPORT, 1),
            Character("Controller1", CharacterRole.CONTROLLER, 1),
        ]
        bad_party = Party(1, "Bad Party", bad_members)
        print("   ERROR: Should have failed!")
    except ValueError as e:
        print(f"   ✓ Correctly rejected invalid composition: {e}")
    
    # Test 8: Test expedition summary
    print("\n8. Testing expedition summary...")
    party.add_gold(200)
    party.complete_floor()
    summary = party.get_expedition_summary()
    print(f"   Summary: {summary}")
    
    print("\n✓ All tests passed! Party model is working.")
